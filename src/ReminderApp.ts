...
export default class ReminderApp {
  ...
  private handleToggleCompletion(): void {
    /*
        ...
        */
    if (!this._remindersHandler.reminders[0]) {
      console.log('\n ⚠️  You have no reminders')
    } else {
      Logger.logReminders(this._remindersHandler.reminders);
      const toggle = this.getUserChoice("reminder number to toggle", true)
      this._remindersHandler.toggleCompletion(parseInt(toggle))
      Logger.log('\n  🏁   Reminder Completion Toggled');
    }
  }
  ...
  private handleModifyReminders(): void {
    ...
    if (!this._remindersHandler.reminders[0]) {
      ...
      Logger.logReminders(this._remindersHandler.reminders);
      const edit = this.getUserChoice("reminder number to edit", false)

      // ERRRROOORRR HANDLE

      const newDescription = this.getUserChoice("reminder description", false)
      this._remindersHandler.modifyReminder(parseInt(edit), newDescription)
      if (ReminderApp.checkUserToggleChoice()) {
        this._remindersHandler.toggleCompletion(parseInt(edit))
        console.log("reminder has been toggle")
      }
  ...
  private handleSearchReminders(): void {
    ...
    */
    if (!this._remindersHandler.reminders[0]) {
      console.log('\n ⚠️  Ysou have no reminders')
    } else {
      const result = this.getUserChoice("search keyword", false)
      const match = this._remindersHandler.search(result)
      Logger.logSearchResults(match)
    }
  }
  ...
  private handleShowReminders(): void {
    let reminderStatus = this._remindersHandler.reminders[0]
    if (!this._remindersHandler.reminders[0]) {
      console.log('\n ⚠️  You have no reminders')
    } else {
